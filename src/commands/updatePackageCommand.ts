...
export function registerUpdatePackageCommands(services: GetllServices): vscode.Disposable[] {
  const updateOne = vscode.commands.registerCommand("getll.updatePackage", async () => {
    ...
    if (using.length === 0) {
      vscode.window.showInformationMessage(`NuGet LL: ${packageId} is not referenced by any project.`);
      return;
    }
  ...
  const updateAll = vscode.commands.registerCommand("getll.updateAllPackages", async () => {
    ...
    if (!outdated || outdated.length === 0) {
      vscode.window.showInformationMessage("NuGet LL: all packages are up to date.");
      return;
    }
    ...
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: "NuGet LL: Updating packages", cancellable: true },
    ...
    if (failures === 0) {
      vscode.window.showInformationMessage("NuGet LL: all updates applied.");
    } else {
      vscode.window.showErrorMessage(`NuGet LL: ${failures} update(s) failed. See output for details.`);
    }
