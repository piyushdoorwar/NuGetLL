...
export async function pickProjects(
  ...
  if (writable.length === 0) {
    vscode.window.showWarningMessage("GetLL: no writable .NET projects found in this workspace.");
    return undefined;
  }
...
export async function pickPackage(services: GetllServices): Promise<string | undefined> {
  ...
  try {
    const results = await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: `GetLL: Searching "${query}"...` },
      () => services.api.searchPackages(query, { includePrerelease: config.includePrerelease, take: config.maxSearchResults })
    );
    if (results.length === 0) {
      vscode.window.showInformationMessage(`GetLL: no packages found for "${query}".`);
      return undefined;
    }
