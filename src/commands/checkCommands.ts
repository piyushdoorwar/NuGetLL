...
function reportErrors(errors: string[]): void {
  ...
  if (errors.length > 0) {
    vscode.window.showWarningMessage("NuGet LL: some projects could not be analyzed. See output for details.");
  }
}
...
export async function runOutdatedCheck(services: GetllServices): Promise<OutdatedPackage[] | undefined> {
  if (!services.dotnet.available) {
    vscode.window.showErrorMessage("NuGet LL: dotnet SDK not found.");
    return undefined;
  }
  const targets = listTargets(services);
  if (targets.length === 0) {
    vscode.window.showInformationMessage("NuGet LL: no .NET projects found.");
    return undefined;
  }
  const config = getConfig();
  let results: OutdatedPackage[] | undefined;
  await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: "NuGet LL: Checking outdated packages", cancellable: true },
...
export function registerCheckCommands(services: GetllServices): vscode.Disposable[] {
  const outdated = vscode.commands.registerCommand("getll.checkOutdated", async () => {
    ...
    if (results) {
      vscode.window.showInformationMessage(
        results.length === 0 ? "NuGet LL: all packages are up to date." : `NuGet LL: ${results.length} outdated package(s) found.`
      );
    }
  });
  ...
  const vulnerable = vscode.commands.registerCommand("getll.checkVulnerable", async () => {
    ...
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: "NuGet LL: Checking vulnerable packages", cancellable: true },
      async (_progress, token) => {
        const abort = new AbortController();
        token.onCancellationRequested(() => abort.abort());
        ...
        vscode.window.showInformationMessage(
          outcome.results.length === 0
            ? "NuGet LL: no known vulnerabilities found."
            : `NuGet LL: ${outcome.results.length} vulnerable package reference(s) found.`
        );
      }
    );
  });
  ...
  const deprecated = vscode.commands.registerCommand("getll.checkDeprecated", async () => {
    ...
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: "NuGet LL: Checking deprecated packages", cancellable: true },
      async (_progress, token) => {
        const abort = new AbortController();
        token.onCancellationRequested(() => abort.abort());
        ...
        if (outcome.results.length === 0) {
          vscode.window.showInformationMessage("NuGet LL: no deprecated packages found.");
        } else {
          ...
          vscode.window
            .showWarningMessage(`NuGet LL: ${outcome.results.length} deprecated package(s) found.`, "Show Details")
            .then((choice) => {
