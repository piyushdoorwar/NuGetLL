...
function summarize(outcome: OperationOutcome, verb: string, packageId: string): void {
  ...
  if (failed === 0 && ok > 0) {
    ...
    vscode.window.showInformationMessage(`NuGet LL: ${verb} ${packageId} in ${ok} project(s)${skippedNote}.`);
  } else if (failed > 0) {
    vscode.window
      .showErrorMessage(
        `NuGet LL: failed to ${verb.toLowerCase()} ${packageId} in ${failed} project(s). See output for details.`,
        "Open Output"
      )
      .then((choice) => {
    ...
    vscode.window.showWarningMessage(`NuGet LL: nothing changed — ${skipped} project(s) skipped.`);
  }
}
...
export async function installPackage(
  ...
  if (!services.dotnet.available) {
    vscode.window.showErrorMessage("NuGet LL: dotnet SDK not found. Package actions are disabled.");
    return outcome;
  }
  ...
  if (cpmProjects.length > 0 && !cpmVersion) {
    ...
    if (!cpmVersion) {
      ...
      vscode.window.showErrorMessage(
        `NuGet LL: could not determine the latest version of ${packageId} for centrally managed projects.`
      );
      return outcome;
    }
  }
  ...
  await vscode.window.withProgress(
    {
      ...
      title: `NuGet LL: ${verb} ${packageId}${version ? ` ${version}` : ""}`,
      cancellable: true
    },
...
export async function removePackage(
  ...
  await vscode.window.withProgress(
    {
      location: vscode.ProgressLocation.Notification,
      title: `NuGet LL: Removing ${packageId}`,
      cancellable: true
    },
    async (progress, token) => {
      const abort = new AbortController();
...
export async function restoreTarget(services: GetllServices, targetPath?: string, label?: string): Promise<boolean> {
  if (!services.dotnet.available) {
    vscode.window.showErrorMessage("NuGet LL: dotnet SDK not found. Restore is disabled.");
    return false;
  }
  if (targetPath) {
  ...
  await vscode.window.withProgress(
    {
      location: vscode.ProgressLocation.Notification,
      title: `NuGet LL: Restoring ${label ?? targetPath ?? "workspace"}`,
      cancellable: true
    },
    async (_progress, token) => {
      ...
      if (success) {
        vscode.window.showInformationMessage(`NuGet LL: restore completed for ${label ?? targetPath ?? "workspace"}.`);
      } else {
        vscode.window
          .showErrorMessage("NuGet LL: restore failed. See output for details.", "Open Output")
          .then((choice) => {
            if (choice === "Open Output") {
              logger.show();
            }
